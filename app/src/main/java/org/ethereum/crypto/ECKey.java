...
public class ECKey implements Serializable {
    private static final BigInteger SECP256K1N = new BigInteger("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141",
            16);
    ...
    public static class ECDSASignature {
         ...
         * modify the bits of a Ethereum transaction after it's been signed, as
    ...
    public BigInteger keyAgreement(ECPoint otherParty) {
        if (privKey == null) {
            ...
            try {
                ...
                throw new RuntimeException("ECDH key agreement failure", e1);
            }catch (  InvalidKeyException e2) {
            	throw new RuntimeException("ECDH key agreement failure", e2);
			}catch (InvalidKeySpecException e3) {
				throw new RuntimeException("ECDH key agreement failure", e3);
			}
        }
    }
    ...
    public byte[] getPrivKeyBytes() {
        if (privKey == null) {
            ...
            return bigIntegerToBytes(((ECPrivateKey) privKey).getS(), 32);
        } else {
