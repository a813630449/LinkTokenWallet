...
public class ConvertUtils {
    ...
    public static BigInteger unifiedNumericToBigInteger(String number) {
        ...
        if (!match)
            ...
            byte[] numberBytes = hexStringToBytes(number);
            return (new BigInteger(1, numberBytes));
        }
    }
    public static String weiToInt(String number) {
    	BigInteger integer=unifiedNumericToBigInteger(number);
    	BigDecimal bigDecimal=new BigDecimal("1000000000000000000");
    	BigDecimal bigDecimal2=new BigDecimal(integer);
		String result=bigDecimal2.divide(bigDecimal).toString();
		return result;
    }
    ...
    public static String format2(String address) {
		String result="账户尾号";
		result+=address.substring(address.length()-4, address.length());
		return result;
	}
